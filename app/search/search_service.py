...
class AppSearchService:
    ...
    def _ensure_index(self):
        count=self.database.query("SELECT COUNT(*) count FROM app_search_index")[0]["count"]
        if not count and self.indexer: self.indexer.reindex()
    def search(self,query:str,filters:dict|None=None,limit:int=30):
        self._ensure_index(); return search_rows(self.database,query,filters,limit)
