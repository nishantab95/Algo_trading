...
def validate_combo(combo:dict,base_ids:set[str]|None=None):
    ...
    for item in components:
        ...
        if kind=="primitive" and ref not in PRIMITIVES: errors.append(f"Unknown primitive: {ref}")
            ...
            import inspect
            args=item.get("args",[]); args=args if isinstance(args,list) else [args]
            try: inspect.signature(PRIMITIVES[ref]).bind(None,*args)
            except TypeError as exc: errors.append(f"Bad parameters for {ref}: {exc}")
